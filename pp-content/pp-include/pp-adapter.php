<?php
    ...
    $professorpay_current_version = [
        'version_name' => 'v' . $dynVersion,
        'version_code' => $dynVersion,
...
09a03d6d70021d1c0dd64cefd6e400b18d0e43d00d821b8f52e2e9370908779e',
    ...
    $directory = ltrim(rtrim(dirname($_SERVER['PHP_SELF']), '/\\') . '/', '/');
    $site_url = pp_site_url('fulldomain') . '/' . $directory;

    $temp_brand_query = json_decode(getData($db_prefix.'brands', 'LIMIT 1'), true);
    $default_brand = $temp_brand_query['status'] == true ? $temp_brand_query['response'][0] : null;

    $professorpay_favicon = (isset($default_brand['favicon']) && !empty($default_brand['favicon']) && $default_brand['favicon'] !== '--') ? $default_brand['favicon'] : $site_url . 'assets/images/favicon-light.png';
    $professorpay_logo_light = (isset($default_brand['logo']) && !empty($default_brand['logo']) && $default_brand['logo'] !== '--') ? $default_brand['logo'] : $site_url . 'assets/images/logo-light.png';
    $professorpay_logo_dark = $site_url . 'assets/images/logo-dark.png';
...
<?php
        ...
        if($action == ""){
            ...
            if($action == "paymentLink-create"){
                if($global_user_login == true){
                    ...
                    $status = escape_string($_POST['status'] ?? '');
                    $return_url = escape_string($_POST['return_url'] ?? '');
                    
                    $is_temporary = isset($_POST['is_temporary']) && $_POST['is_temporary'] == '1' ? 1 : 0;
                    $allowed_gateways = isset($_POST['allowed_gateways']) && is_array($_POST['allowed_gateways']) ? $_POST['allowed_gateways'] : [];
                    ...
                    if($title == "" || $currency == "" || $amount == "" || $status == ""){
                        echo json_encode(['status' => "false", 'title' => 'Incomplete Information', 'message' => 'Please fill in all required fields before proceeding.', 'csrf_token' => $new_csrf_token]);
                    }else{
                        ...
                        if (isset($_FILES['image']) && $_FILES['image']['error'] == UPLOAD_ERR_OK) {
                            $storage = __DIR__ . '/../../pp-media/storage/payment-links/';
                            if (!is_dir($storage)) @mkdir($storage, 0755, true);
                            
                            $extension = pathinfo($_FILES['image']['name'], PATHINFO_EXTENSION);
                            $filename = 'pl_' . uniqid() . '.' . $extension;
                            $destination = $storage . $filename;
                            
                            if (move_uploaded_file($_FILES['image']['tmp_name'], $destination)) {
                                $image_url = $site_url . 'pp-media/storage/payment-links/' . $filename;
                            }
                        }
                        ...
                        $columns = ['ref', 'brand_id', 'product_info', 'amount', 'quantity', 'currency', 'expired_date', 'status', 'return_url', 'created_date', 'updated_date'];
                        $values = [$paymentLinkID, $global_response_brand['response'][0]['brand_id'], $product_info, money_sanitize($amount), money_sanitize($quantity), $currency, $expiry_date, $status, $return_url, getCurrentDatetime('Y-m-d H:i:s'), getCurrentDatetime('Y-m-d H:i:s')];
            ...
            if($action == "paymentLink-edit"){
                if($global_user_login == true){
                    ...
                    if($paymentLinkID == "" || $title == "" || $currency == "" || $amount == "" || $status == ""){
                        echo json_encode(['status' => "false", 'title' => 'Incomplete Information', 'message' => 'Please fill in all required fields before proceeding.', 'csrf_token' => $new_csrf_token]);
                    }else{
                        $response_pl = json_decode(getData($db_prefix.'payment_link','WHERE ref = "'.$paymentLinkID.'"'),true);
                        $existing_info = json_decode($response_pl['response'][0]['product_info'] ?? '{}', true);
                        $image_url = $existing_info['image'] ?? '';
                        ...
                        $columns = ['product_info', 'amount', 'quantity', 'currency', 'expired_date', 'status', 'return_url', 'updated_date'];
                        $values = [$product_info, money_sanitize($amount), money_sanitize($quantity), $currency, $expiry_date, $status, $return_url, getCurrentDatetime('Y-m-d H:i:s')];
            ...
            if($action == "paymentLink-defaultLinkLogo"){
                if($global_user_login == true){
                    if (!canAccessPage(json_decode($global_response_permission['response'][0]['permission'], true), 'payment_link', $global_user_response['response'][0]['role'])) {
                        echo json_encode(['status' => 'false', 'title' => 'Access denied', 'message' => 'You need permission to perform this action. Please contact the admin.' , 'csrf_token' => $new_csrf_token]);
                        exit();
                    ...
                    $max_file_size = 2 * 1024 * 1024; 
                    $logo_upload = json_decode(uploadImage($_FILES['logo_file']?? null, $max_file_size), true);

                    if($logo_upload['status'] == true){
                        $logo_url = $site_url.'pp-media/storage/'.$logo_upload['file'];
                        
                        $old_logo = get_env('payment-link-default-logo', $global_response_brand['response'][0]['brand_id']);
                        if(!empty($old_logo) && strpos($old_logo, 'pp-media/storage/') !== false) {
                            $old_file = str_replace($site_url.'pp-media/storage/', '', $old_logo);
                            deleteImage($old_file);
                        }

                        set_env('payment-link-default-logo', $logo_url, $global_response_brand['response'][0]['brand_id']);
                        echo json_encode(['status' => 'true', 'title' => 'Logo Updated', 'message' => 'The default payment link logo has been updated successfully.', 'csrf_token' => $new_csrf_token]);
                    }else{
                        echo json_encode(['status' => 'false', 'title' => 'Upload Failed', 'message' => $logo_upload['message'] ?? 'Failed to upload logo.', 'csrf_token' => $new_csrf_token]);
                    }
                }else{
                    echo json_encode(['status' => 'false', 'title' => 'Request Failed', 'message' => 'Invalid request' , 'csrf_token' => $new_csrf_token]);
                }
            ...
            if($action == "paymentLink-removeDefaultLogo"){
                if($global_user_login == true){
                    if (!canAccessPage(json_decode($global_response_permission['response'][0]['permission'], true), 'payment_link', $global_user_response['response'][0]['role'])) {
                        echo json_encode(['status' => 'false', 'title' => 'Access denied', 'message' => 'You need permission to perform this action.' , 'csrf_token' => $new_csrf_token]);
                        exit();
                    ...
                    set_env('payment-link-default-logo', '--', $global_response_brand['response'][0]['brand_id']);
                    echo json_encode(['status' => 'true', 'title' => 'Logo Removed', 'message' => 'The default link logo has been removed successfully.', 'csrf_token' => $new_csrf_token]);
                }else{
                    echo json_encode(['status' => 'false', 'title' => 'Request Failed', 'message' => 'Invalid request' , 'csrf_token' => $new_csrf_token]);
                }
            }
            ...
            if($action == "transaction-list"){
                if($global_user_login == true){
                    ...
                    if ($search_input !== '') {
                        $sql_query .= " AND ( ref LIKE '%$search_input%' OR customer_info LIKE '%$search_input%' OR trx_id LIKE '%$search_input%' OR gateway_id LIKE '%$search_input%' OR sender LIKE '%$search_input%' )";
                    }
                    ...
                    if($response_result['status'] == true){
                        ...
                        foreach($response_result['response'] as $row){
                            ...
                            $is_temporary = isset($metadata['is_temporary']) && $metadata['is_temporary'] == 1;
            ...
            if($action == "system-settings-update-check"){
                if($global_user_login == true){
                    if (!empty($pp_demo_mode)) {
                        ...
                        $version_hash = $professorpay_current_version['version_hash'];
            ...
            if($action == "system-settings-update-download"){
                if($global_user_login == true){
                    if (!empty($pp_demo_mode)) {
                        ...
                        if (version_compare($lasted_update_version, $professorpay_current_version['version_code'], '>')) {
                            $update_available = true;
                        }
                ...
                if($global_user_login == true){
                    if (!empty($pp_demo_mode)) {
                        ...
                        if($update_available == true){
                            ...
                            zipFolder($root, "$backupDir/".$professorpay_current_version['version_code'].".zip");

                            backupDatabasePDO("$backupDir/db_".$professorpay_current_version['version_code'].".sql");
                    ...
                    if($gateway == ""){
                        ...
                        if (!file_exists(__DIR__ . '/../pp-modules/pp-gateways/'.$gateway.'/class.php')) {
                            ...
                            if (!class_exists($class)) {
                                ...
                                echo json_encode(['status' => 'true', 'title' => 'Gateway Created', 'message' => 'The gateway has been created successfully.', 'csrf_token' => $new_csrf_token, 'gateway_id' => $gateway_id]);
            ...
            if($action == "gateways-list"){
                if($global_user_login == true){
                    ...
                    $custom_order = "CASE WHEN LOWER(name) LIKE '%bkash%' THEN 1 WHEN LOWER(name) LIKE '%nagad%' THEN 2 WHEN LOWER(name) LIKE '%rocket%' THEN 3 WHEN LOWER(name) LIKE '%binance%' THEN 4 WHEN LOWER(name) LIKE '%bank%' THEN 5 ELSE 6 END ASC, 1 DESC";
                    $response_result = json_decode(getData($db_prefix.'gateways',' WHERE '.$where_sql.' brand_id ="'.$global_response_brand['response'][0]['brand_id'].'" '.$sql_query.' ORDER BY '.$custom_order.$sql_limit),true);
                    if($response_result['status'] == true){
                        $response = [];

                        foreach($response_result['response'] as $row){
                            $response_mobile = json_decode(getData($db_prefix.'gateways_parameter', 'WHERE gateway_id = "'.$row['gateway_id'].'" AND brand_id = "'.$global_response_brand['response'][0]['brand_id'].'" AND option_name = "mobile_number"'), true);
                            $response_account = json_decode(getData($db_prefix.'gateways_parameter', 'WHERE gateway_id = "'.$row['gateway_id'].'" AND brand_id = "'.$global_response_brand['response'][0]['brand_id'].'" AND option_name = "account_number"'), true);
                            
                            $subtitle = $row['display'];
                            if($response_mobile['status'] == true && !empty($response_mobile['response'][0]['value']) && $response_mobile['response'][0]['value'] != '--'){
                                $subtitle = $response_mobile['response'][0]['value'];
                            } else if($response_account['status'] == true && !empty($response_account['response'][0]['value']) && $response_account['response'][0]['value'] != '--'){
                                $subtitle = $response_account['response'][0]['value'];
                            }
    ...
    if(isset($_POST['action-v2'])){
        ...
        if($action == ""){
            ...
            if($action == "payment-link"){
                ...
                if($itemid == ""){
                    ...
                    if($response_payment_link['status'] == true){
                        ...
                        if($paymentRow['quantity'] == -1 || $paymentRow['quantity'] > 0){
                        ...
                        $metadata_array = [
                            'paymentLink_id' => $itemid
                        ];
                        
                        $product_info = json_decode($paymentRow['product_info'] ?? '{}', true);
                        if (isset($product_info['is_temporary']) && $product_info['is_temporary'] == 1) {
                            $metadata_array['is_temporary'] = true;
                            $metadata_array['allowed_gateways'] = $product_info['allowed_gateways'] ?? [];
                        }
                        ...
                        $return_url = $paymentRow['return_url'] ?? '';
                        $columns = ['brand_id', 'source', 'ref', 'customer_info', 'amount', 'currency', 'source_info', 'metadata', 'return_url', 'created_date', 'updated_date'];
            ...
            if($action == "transaction-verify-auto"){
    $gateway_id = escape_string($_POST['gateway-id'] ?? '');
    $transaction_id = trim(escape_string($_POST['transaction-id'] ?? ''));
    $mobile_number = trim(escape_string($_POST['mobile_number'] ?? ''));

    if($gateway_id == "" || $transaction_id == ""){
        echo json_encode(['status' => "false", 'title' => 'Incomplete Information', 'message' => 'Waiting for payment...', 'pending' => true]);
        exit;
    }
            ...
            if($response_gateway['status'] == true){
                ...
                if(isset($gateway_info) && $gateway_info['gateway_type'] == "automation"){
                    ...
                    if ($matched_sms) {
                            ...
                            $condition = 'id ="'.$matched_sms['id'].'"'; 
                            updateData($db_prefix.'sms_data', $columns, $values, $condition);

                            // Update Transaction
                            $trxid = $matched_sms['trx_id'];
                            ...
                            $response_transaction2 = json_decode(getData($db_prefix.'transaction','WHERE ref = :ref AND status = :status ', '* FROM', $params),true);
                            ...
                            if($response_transaction2['response'][0]['webhook_url'] != "" && $response_transaction2['response'][0]['webhook_url'] != "--"){
                                ...
                                foreach ($jobs as $job) {
                                    ...
                                    if($status != 'completed'){
                                        $columns = ['ref', 'brand_id', 'payload', 'url', 'created_date', 'updated_date'];
                                        $values = [rand(), $response_brand['response'][0]['brand_id'], $payload, $response_transaction2['response'][0]['webhook_url'], getCurrentDatetime('Y-m-d H:i:s'), getCurrentDatetime('Y-m-d H:i:s')];
                                        insertData($db_prefix.'webhook_log', $columns, $values);
                                    }
                                }
                            }
    ...
    echo json_encode(['status' => "false", 'title' => 'Pending', 'message' => 'Waiting for payment...', 'pending' => true]);
    exit;
}
                                    ...
                                    if(isset($gateway_info)){
                                        ...
                                        if(isset($gateway_info['gateway_type']) && $gateway_info['gateway_type'] == "automation"){
                                            ...
                                            if ($verification_method === 'number_amount') {
                                                ...
                                                if ($mobile_number === '') {
                                                    echo json_encode(['status' => "false", 'title' => 'Missing Number', 'message' => 'Please enter your mobile number to verify.']);
                                                } else {
                                                    $search_number = '%'.$mobile_number.'%';
                                                    if ($gateway_info['sender_key'] === 'rocket') {
                                                        $search_number = '%'.substr($mobile_number, -3).'%';
                                                    ...
                                                    $matched_sms = null;
                                                    if ($response_sms['status'] == true && !empty($response_sms['response'])) {
                                                        $response_brand_tol = json_decode(getData($db_prefix.'brands', ' WHERE brand_id ="'.$response_transaction['response'][0]['brand_id'].'"'), true);
                                                        $tolerance = $response_brand_tol['response'][0]['payment_tolerance'] ?? 0;
                                                        foreach ($response_sms['response'] as $sms) {
                                                            if (verifyPaymentTolerance($convertedAmount, $sms['amount'], $tolerance)) {
                                                                $matched_sms = $sms;
                                                                break;
                                                            }
                                                        }
                                                    ...
                                                    if ($matched_sms) {
                                                        // Mark SMS as used
                                                        updateData($db_prefix.'sms_data', ['status','updated_date'], ['used', getCurrentDatetime('Y-m-d H:i:s')], 'id ="'.$matched_sms['id'].'"');

                                                        // Complete transaction
                                                        $matched_trxid = $matched_sms['trx_id'] ?? '--';
                                                        $columns = ['processing_fee','discount_amount','local_net_amount','local_currency','gateway_id','sender_key','status','sender','trx_id','updated_date'];
                                                        ...
                                                        if (!isset($_SESSION['verify_attempt_'.$txn_id])) {
                                                            $_SESSION['verify_attempt_'.$txn_id] = 1;
                                                            echo json_encode(['status' => "false", 'title' => 'Verification Failed', 'message' => 'We could not find a matching payment for this Mobile Number. Please check and try again.']);
                                                        } else {
                                            ...
                                            // ── TRXID MODE (default) ────────────────────────────────────────
                                                ...
                                                if ($trxid === '') {
                                                    ...
                                                    // Check duplicate
                                                    $params = [':trx_id' => $trxid];
                                                    $dupCheck = json_decode(getData($db_prefix.'transaction', 'WHERE trx_id = :trx_id', '* FROM', $params), true);
                                                    if ($dupCheck['status'] == true) {
                                                        echo json_encode(['status' => "false", 'title' => 'Duplicate Transaction ID', 'message' => 'This Transaction ID already exists. Please provide a different one.']);
                                                    } else {
                                                        // Match SMS by trxid + amount
                                                        $params = [
                                                            ':sender_key' => $gateway_info['sender_key'],
                                                            ':type'       => $gateway_info['sender_type'],
                                                            ':trx_id'     => $trxid,
                                                        ...
                                                        if ($matched_sms) {
                                                            ...
                                                            if (!isset($_SESSION['verify_attempt_'.$txn_id])) {
                                                                $_SESSION['verify_attempt_'.$txn_id] = 1;
                                                                echo json_encode(['status' => "false", 'title' => 'Verification Failed', 'message' => 'We could not find a matching payment for this Transaction ID. Please check and try again.']);
                                                            } else {
                                                                unset($_SESSION['verify_attempt_'.$txn_id]);
                                                                // No matching SMS — push to pending
