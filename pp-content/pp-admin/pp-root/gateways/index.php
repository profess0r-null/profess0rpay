...
<script data-cfasync="false">
                    ...
                    res.response.forEach(item => {
                        ...
                        html += `
                            <tr data-id="${item.id}">
                                ...
                                <td>
                                    <button class="btn btn-sm btn-${item.status === 'active' ? 'success' : 'danger'}" onclick="toggleGatewayStatus('${item.id}', '${item.status === 'active' ? 'inactive' : 'active'}')" style="padding: 2px 8px; font-size: 12px; height: auto;">
    ...
    function toggleGatewayStatus(itemID, newStatus) {
        var csrf_token_default = $('input[name="csrf_token_default"]').val();
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo $site_url.$path_admin ?>/dashboard',
            data: {action: "gateways-bulk-action", csrf_token: csrf_token_default, actionID: (newStatus === 'active' ? 'activated' : 'inactivated'), selected_ids: JSON.stringify([itemID])},
            dataType: 'json',
            success: function (response) {
                document.querySelector('.'+loaderSpinner).innerHTML = '';
